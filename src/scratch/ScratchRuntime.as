...
//----------
	...
	public function allVarNames():Array {
		...
		if ((app.usesUserNameBlock || usernameValue.length > 0) && result.indexOf('username') == -1) {
	...
	public function getAllVars():Array {
		...
		if ((app.usesUserNameBlock || usernameValue.length > 0) && !hasVar(result, 'username')) {
			result.push({name: 'username', value: usernameValue});
		}
		return result;
	}

	private function hasVar(vars:Array, name:String):Boolean {
		for each (var entry:Object in vars) {
			if (entry.name == name) return true;
		}
		return false;
	}

	public function setVarValue(varName:String, value:*):void {
		if (varName == null) return;
		if (varName.toLowerCase() == 'username') {
			usernameValue = (value != null) ? String(value) : '';
			return;
		}
		var variable:Variable = app.viewedObj().lookupVar(varName);
		if (variable == null) {
			variable = lookupClosestVar(varName);
		}
		if (variable == null) {
			variable = app.viewedObj().lookupOrCreateVar(varName);
		}
		if (variable != null) {
	...
	private function lookupClosestVar(varName:String):Variable {
		if (varName == null) return null;
		var lowerName:String = varName.toLowerCase();
		var best:Variable = null;
		var bestScore:int = int.MAX_VALUE;

		for each (var v:Variable in app.viewedObj().variables) {
			if (v.name.toLowerCase() == lowerName) return v;
			if (v.name.toLowerCase().indexOf(lowerName) == 0) return v;
			var score:int = stringDistance(lowerName, v.name.toLowerCase());
			if (score < bestScore) { bestScore = score; best = v; }
		}
		for each (v in app.stageObj().variables) {
			if (v.name.toLowerCase() == lowerName) return v;
			if (v.name.toLowerCase().indexOf(lowerName) == 0) return v;
			var score2:int = stringDistance(lowerName, v.name.toLowerCase());
			if (score2 < bestScore) { bestScore = score2; best = v; }
		}

		if (best != null && bestScore <= Math.max(3, lowerName.length / 2)) {
			return best;
		}
		return null;
	}

	private function stringDistance(a:String, b:String):int {
		var costs:Array = [];
		for (var i:int = 0; i <= a.length; i++) {
			costs[i] = i;
		}
		for (var j:int = 1; j <= b.length; j++) {
			var lastCost:int = j - 1;
			costs[0] = j;
			for (i = 1; i <= a.length; i++) {
				var newCost:int = costs[i - 1];
				if (a.charAt(i - 1) != b.charAt(j - 1)) {
					newCost = Math.min(Math.min(newCost + 1, costs[i] + 1), lastCost + 1);
				}
				costs[i - 1] = lastCost;
				lastCost = newCost;
			}
			costs[a.length] = lastCost;
		}

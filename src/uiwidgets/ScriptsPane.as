...
public class ScriptsPane extends ScrollFrameContents {
	...
	private var isDraggingBackground:Boolean = false;
	private var lastMouseX:Number = 0;
	private var lastMouseY:Number = 0;
	private var scrollAccX:Number = 0;
	private var scrollAccY:Number = 0;
	...
	public function ScriptsPane(app:Scratch) {
		...
		createTexture();
		addFeedbackShape();
		addEventListener(MouseEvent.MOUSE_DOWN, onPaneMouseDown);
		addEventListener(MouseEvent.MOUSE_MOVE, onPaneMouseMove);
		addEventListener(MouseEvent.MOUSE_UP, onPaneMouseUp);
	}
...
return true; // xxx disable this check for now; it was causing confusion at Scratch@MIT conference
	...
	private function onPaneMouseDown(evt:MouseEvent):void {
		var targetObj:DisplayObject = getChildAtLocalPoint(evt.stageX, evt.stageY, 0, numChildren);
		if (targetObj == null) {
	...
	private function getChildAtLocalPoint(stageX:Number, stageY:Number, startIndex:int, endIndex:int):DisplayObject {
		for (var i:int = endIndex - 1; i >= startIndex; i--) {
			var child:DisplayObject = getChildAt(i);
			if (child is Block || child is ScratchComment) {
	...
	private function onPaneMouseMove(evt:MouseEvent):void {
		if (isDraggingBackground) {
			var deltaX:Number = (evt.stageX - lastMouseX) / -2;
			var deltaY:Number = (evt.stageY - lastMouseY) / -2;
			lastMouseX = evt.stageX;
			lastMouseY = evt.stageY;
			...
			var hDelta:int = int(scrollAccX);
			var vDelta:int = int(scrollAccY);
			if (hDelta != 0) scrollAccX -= hDelta;
			if (vDelta != 0) scrollAccY -= vDelta;

			if (vDelta != 0) {
				var verticalScroll:MouseEvent = new MouseEvent(MouseEvent.MOUSE_WHEEL, true, false,
					evt.localX, evt.localY, null, evt.ctrlKey, evt.altKey, false, evt.buttonDown, vDelta);
				if (parent is ScrollFrame) ScrollFrame(parent).dispatchEvent(verticalScroll);
				else dispatchEvent(verticalScroll);
			}
			if (hDelta != 0) {
				var horizontalScroll:MouseEvent = new MouseEvent(MouseEvent.MOUSE_WHEEL, true, false,
					evt.localX, evt.localY, null, evt.ctrlKey, evt.altKey, true, evt.buttonDown, hDelta);
				if (parent is ScrollFrame) ScrollFrame(parent).dispatchEvent(horizontalScroll);
				else dispatchEvent(horizontalScroll);
			}
		}
	}

	private function onPaneMouseUp(evt:MouseEvent):void {

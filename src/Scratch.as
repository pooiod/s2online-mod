...
public class Scratch extends Sprite {
	...
	private function getAllVarsCallback():Array {
		return runtime.getAllVars();
	}

	private function setVarValueCallback(varName:String, value:*):void {
		runtime.setVarValue(varName, value);
	}

	private function createVariableCallback(varName:String):void {
		runtime.createVariable(varName);
	}
	...
	protected function startInEditMode():Boolean {
		...
		if (ExternalInterface.available) {
			url = String(ExternalInterface.call("function(){return window.location.pathname;}"));
		}
		url = url.split("?")[0].split("#")[0];
		return !(url.indexOf("/player") != -1 || url.indexOf("/embed") != -1);
	}
	...
	private function uncaughtErrorHandler(event:UncaughtErrorEvent):void {
		if (event.error is Error) {
			...
			if (jsEnabled && ExternalInterface.available) {
				ExternalInterface.call("console.error", "Uncaught Error: " + errorEvent.toString());
			}
		}
	}
	...
	public function log(severity:String, messageKey:String, extraData:Object = null):LogEntry {
		if (jsEnabled && ExternalInterface.available) {
			if (severity == LogLevel.ERROR) {
				ExternalInterface.call("console.error", "[" + severity + "] " + messageKey);
			} else if (severity == LogLevel.WARNING) {
				ExternalInterface.call("console.warn", "[" + severity + "] " + messageKey);
			}
		}
	...
	public function logException(e:Error):void {
		...
		if (jsEnabled && ExternalInterface.available) {
			ExternalInterface.call("console.error", "Exception: " + e.toString() + "\n" + e.getStackTrace());
		}
	}
	...
	public function logMessage(msg:String, extra_data:Object = null):void {
		...
		if (jsEnabled && ExternalInterface.available) {
			ExternalInterface.call("console.error", "Error Message: " + msg);
		}
	}
...
//		}
	...
	public function dlFrom2():void {
		if (jsEnabled) {
			externalCall('showProjectPicker', null, '');
		}
	}
